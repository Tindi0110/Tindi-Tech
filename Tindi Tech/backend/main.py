...
# CORS: Allow everything (simplest for now, though less critical with same-origin)
# CORS: Allow everything (simplest for now, though less critical with same-origin)
...
wifi_sessions_col = db["wifi_sessions"]  # For active wifi users
...
def register():
    ...
    # Send Welcome Email (Async recommended for prod, sync for now)
    try:
        if config.MAIL_USERNAME:
            msg = Message(
                subject="Welcome to Tindi Tech!",
                sender=config.MAIL_DEFAULT_SENDER,
                recipients=[email],
                body=f"Hi {data['fname']},\n\nWelcome to Tindi Tech! We're glad to have you.\n\nLogin here: http://localhost:5500/login.html"
            )
            mail.send(msg)
    except Exception as e:
        print(f"Email Error: {e}")  # Non-blocking

    return jsonify({"success": True, "message": f"Registered successfully as {role.replace('_', ' ').title()}"})
...
def get_order_status(order_id):
    ...
    # Auto-complete payment for demo (DEBUG MODE ONLY)
    # FIX: Do not auto-complete if order was canceled by admin
    if config.DEBUG and order.get("payment", {}).get("status") == "pending" and order.get("status") != 'canceled':
        orders_col.update_one({"order_id": order_id}, {"$set": {"payment.status": "paid", "status": "processing"}})
...
def get_wifi_plans():
    """Return hardcoded/dynamic wifi plans."""
    return jsonify({"success": True, "plans": WIFI_PLANS})


@app.route("/wifi/pay", methods=["POST"])
def wifi_pay():
    """Initiate M-Pesa Payment for WiFi (Persistent)."""
    data = request.get_json() or {}
    phone = data.get("phone")
...
def wifi_check_status(checkout_id):
    ...
    if status == "pending_payment" and config.DEBUG:
         # Simulate Success
         mpesa_code = "MPESA" + str(random.randint(1000,9999))
         wifi_sessions_col.update_one(
             {"_id": session["_id"]}, 
             {"$set": {"status": "paid", "mpesa_code": mpesa_code, "paid_at": datetime.datetime.now()}}
         )
         # Refresh local var
         status = "paid"
         session["mpesa_code"] = mpesa_code
